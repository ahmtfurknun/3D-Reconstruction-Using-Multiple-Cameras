function [point_matrix, sift_points] = n_view_matching(points, features, ima, MaxRatio, Metric)
% n_view_matching Computes point matches along N views. Obtains consistent 
% point correspondences among all images

% It accepts a cell structure with the detected points in each camera (following the same fomat of detectSIFTFeatures),
% a cell structure of descriptors in each camera (following the same fomat of extractFeatures), 
% and a cell structure of images (for visualization of the results)
...
% point_matrix: Point correspondences (non-hom coordinates) allong all views (in the same format of the lab sessions of Unit III) 
    ...
    sift_points = cell(1, numImages);

    for j = 1:numImages

        point_matrix(:,:,j) = points{j}.Location';
        sift_points{j} = points{j};
